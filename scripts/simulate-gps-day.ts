/**
 * Simulate a GPS workday for tracker-van-1.
 * Usage: npx ts-node --compiler-options {"module":"CommonJS"} scripts/simulate-gps-day.ts
 */
...
async function ping(at: string, point: { lat: number; lng: number }) {
...
async function main() {
  ...
  console.log(
    'blocks',
    blocks.map((b: { type: string; status: string }) => `${b.type}/${b.status}`),
  );
  const dayView = await (
  ...
  console.log(
    'visits',
    dayView.visits.map(
      (v: { id: string; status: string; dwellMinutes: number; orderLinks: unknown[] }) => ({
        id: v.id,
        status: v.status,
